...
def banner():
    ...
    autoketik (f"""
     ...
     \033[1;0m──────────────────────────┬─────────────────────────
  \033[1;0m┌────────────────────────────┼───────────────────────────┐
  {W}│ Creator{R}:{W}FAZRY              │  Version {R}:{G} 3.0{W}            │
  {W}│ Facebook{R}:{G}Fazry             {W}│ {W} Your Ip {R}:{Y} {ip} {W}│
  {W}│ Team{R}:{W}Execute {biru}Code          {W}│  Perfect Brutal {biru}Sms Tools {W}│
  └────────────────────────────┼───────────────────────────┘""")
...
def start(): # Def Untuk Start Tools
    ...
    if pil == "2":
        ...
        tanya()
    if pil == "3":
        os.system("xdg-open https://wa.me/6283109115523?text=Hallo+Kak+scriptnya+ada+yg+error+mohon+untuk+segera+memperbaiki+nya")
